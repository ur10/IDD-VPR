import sys
from PyQt5.QtWidgets import QApplication, QMainWindow, QWidget, QLabel, QVBoxLayout, QGridLayout, QSplitter, QFrame, QRadioButton, QPushButton, QMessageBox, QHBoxLayout, QButtonGroup, QSizePolicy, QSpacerItem, QFormLayout,QDialog,QGroupBox,QDesktopWidget,QGraphicsPixmapItem,QGraphicsView,QGraphicsScene
...
class InitialGUI(QWidget):
    ...
    def showPage(self):
        ...
        if not hasattr(self, 'page') or not self.page:
            self.page = ImageGridApp(self.route, self.direction)
...
class EnlargedImageWindow(QDialog):
    def __init__(self, images_set1, images_set2, query_image_path, ref_image_path):
        ...
        query_imgs = os.listdir(query_image_path)
        ref_imgs = os.listdir(ref_image_path)
    ...
    def show_images(self):
        ...
        left_image_path = self.images_set1[self.current_index]
        right_image_path = self.images_set2[self.current_index]
...
class ImageGridApp(QMainWindow, QDialog):
    def __init__(self, route, direction):
        ...
        self.ref_image_dir = "/home/ur10/mnt_pt/2023-27-09_modified"
        self.query_image_dir = "/home/ur10/mnt_pt/2023-29-09_modified"
        ...
        self.annotation_path = "/home/ur10/mnt_pt/2023-27-09_modified/annotation_data/annotation.json"
        ...
        # Create the left column for the single image
        left_column = QWidget()
        ...
        self.current_single_image_index = 0  # Track the current single image index
        ...
        if os.path.isfile(self.saved_annotations_file):
            ...
            for key,value in self.match_dict.items():
                self.tagged_ref_image.append(key)
                self.tagged_query_image.append(value)
    ...
    def confirm_click(self):
        ...
        if check:
            self.match_dict[self.single_image_paths[self.current_single_image_index]] = self.image_paths[self.current_set_start_index + index]
            ...
            self.tagged_ref_image.append(self.single_image_paths[self.current_single_image_index])
            self.tagged_query_image.append(self.image_paths[self.current_set_start_index + index])
    ...
    def load_single_image(self):
        if 0 <= self.current_single_image_index < len(self.single_image_paths):
            ...
            image = self.ref_images_np_list[self.current_single_image_index%100]
            if image is not None:
        ...
        if (self.current_single_image_index+10)%100 == 0:
            ...
            if self.current_single_image_index in self.match_dict or True:
                ...
                if self.current_single_image_index >= len(self.single_image_paths):
                    self.current_single_image_index = len(self.single_image_paths) - 1
                self.load_single_image()
            else:
    ...
    def load_image(self, index):
        ...
        if 0 <= self.current_set_start_index + index < len(self.image_paths):
            ...
            print(f"The loaded index is {(self.current_set_start_index+index)%100}")
            image = query_list[(self.current_set_start_index+index)%100]
            if image is not None:
                return image
    ...
    def show_next_image_set(self):
        ...
        if (self.current_set_start_index+10)%100 == 0:
            # print("Loading images from the buffer")
            if len(self.buffer_query_image_list) != 0 and not self.query_loading:
                self.query_images_np_list = self.buffer_query_image_list[:]
                # self.buffer_query_image_list[:200] = self.buffer_query_image_list[100:300]
                print(f"Loading images from the buffer and the list size is {len(self.query_images_np_list)}")
                self.buffer_query_image_list = []
                self.current_load = "query"
                ...
                #     self.current_set_start_index = len(self.image_paths) - 10
                
                self.load_images(self.query_image_path, self.image_paths, self.current_set_start_index+100, self.current_set_start_index+200)
                self.load_image_set()
            else:
                print("The buffer is empty")
                QMessageBox.critical(self, f"Wait!!!!", "Still loading images query message is {self.query_loading}")
            ...
            # if self.current_set_start_index > 100:
            #     self.current_set_start_index = 100 - 10
